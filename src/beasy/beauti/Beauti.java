...
public class Beauti extends beast.app.beauti.Beauti implements BeautiDocListener {
...
//    }
	...
	private void createFileMenu() {
        ...
        for (AbstractAction a : workDirMenuActions) {
        	workDirMenu.add(a);
            String name = a.NAME;
            workingdir.append("<li><a tabindex=\"0\" >" + name +"</a></li>\n");
        }
        templatedHTML = templatedHTML.replaceAll("\\$\\(workingdir\\)", workingdir.toString());
	...
	private void addAlignmentProviderMenus(JMenu fileMenu) {
        ...
        for (BeautiAlignmentProvider provider : providers) {
        	...
        	imports.append("<li><a tabindex=\"0\" >" + provider.toString() +"</a></li>\n");
        }
        templatedHTML = templatedHTML.replaceAll("\\$\\(templates\\)", imports.toString());
	}
    ...
    public void setUpPanels() throws NoSuchMethodException, SecurityException, ClassNotFoundException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
    	...
    	List<String> panelLabels = new ArrayList<>(); 
        isInitialising = true;
        ...
        for (int panelIndex = 0; panelIndex < doc.beautiConfig.panels.size(); panelIndex++) {
            ...
            panelLabels.add(panelConfig.getName());
        }
        ...
        for (int panelIndex = 0; panelIndex < doc.beautiConfig.panels.size(); panelIndex++) {
        	String tab = doc.beautiConfig.panels.get(panelIndex).getName();
    		
        	viewMenu.append("<li style=\"padding-left:10px;\"><input id=\"view-" + tab + "\" type=\"checkbox\" onclick=\"toggle('" + tab + "')\" "
    				 + (isPaneIsVisible[panelIndex] ? "checked" : "") + "> " + tab + "</input></li>");
    		
    		tabs.append("<li class=\"nav-item\" id=\"" + tab + "-nav\">\n");
    		tabs.append("    <a class=\"nav-link active\" id=\"" + tab + "-tab\" data-toggle=\"tab\" href=\"#" + tab + "\" role=\"tab\" aria-controls=\"" + tab + "\" aria-selected=\"true\">" + tab + "</a>\n");
    		tabs.append("</li>\n");
    		
    		content.append("<div class=\"tab-pane active\" id=\"" + tab + "\" role=\"tabpanel\" aria-labelledby=\"" + tab + "-tab\">" + tab + "</div>");
    	}

    	templatedHTML = baseHTML;
    	templatedHTML = templatedHTML.replaceAll("\\$\\(viewMenu\\)", viewMenu.toString());
    	templatedHTML = templatedHTML.replaceAll("\\$\\(tabs\\)", tabs.toString());
    	templatedHTML = templatedHTML.replaceAll("\\$\\(content\\)", content.toString());
    ...
    String templatedHTML;
	static String baseHTML;
	
	static {
		InputStream in = BeasyBeautiServer.class.getResourceAsStream("/beasy/beauti/beauti.html"); 
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            FileWriter outfile = new FileWriter(new File("/tmp/beauti.html"));
            outfile.write(beauti.templatedHTML);
            outfile.close();
            System.exit(0);
